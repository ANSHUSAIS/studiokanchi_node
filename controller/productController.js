...
exports.getAllProduct = async(req, res, next) => {
    const product = await Product.find({}, { price: 0, splprice: 0 })
        .populate({
            path: "category",
            model: Category,
        })
    ...
    try {
        ...
        if (req.files) {
            ...
            if (req.files.multipleSelectedFile) {
                ...
                for (
                    let index = 0; index < multipleimage.length; index++
                ) {
                    let result;
...
exports.getProductsByAttributes = async(req, res, next) => {
    try {
        ...
        const attributes = await Productattribute.findOne({ product_id: req.params.product_id, attribute_name: req.params.attribute_name, attribute_value: req.params.attribute_value });
